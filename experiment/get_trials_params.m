function [vDirection,vWg,vTask,vOnset,vPhase,vOrient] = get_trials_params(params)
...
for i=1:length(vTask)
    ...
    block_array = [ones(params.trialsPerBlock/4,1) ones(params.trialsPerBlock/4,1); ...
        ...
        ones(params.trialsPerBlock/4,1) zeros(params.trialsPerBlock/4,1); ...
        zeros(params.trialsPerBlock/4,1) zeros(params.trialsPerBlock/4,1)];
    % if this is a discrimination block, all trials should be 'present'
    if ~detection
        block_array(:,1)=1;
    else % if detection, all orientations should be 1
        block_array(:,2) = 1;
    end
...
%randomize phase of gratings
vPhase = rand(size(vWg));

%randomize orientation
vOrient = normrnd(0,1,size(vWg));
